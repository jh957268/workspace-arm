...
CSqlite::insert_user_tag(char *tag, int antid, int action)
{
	...
	sprintf(sql_buff, "select tag_id,tag_seen_count from TAG_USER_DATA where tag_val='%s' AND tag_antID=%d;", tag, antid);
	parm[0] = parm[1] = 0;
	rc = sqlite3_exec(rfid_db, sql_buff, callback, (void*)parm, &zErrMsg);
	...
	{
		int seen_cnt = parm[1] + 1;
		sprintf(sql_buff, "update TAG_USER_DATA SET tag_action=%d,tag_seen_count=%d,tag_last_seen=datetime('now') where tag_id=%d;", action, seen_cnt, parm[0]);
		rc = sqlite3_exec(rfid_db, sql_buff, callback, 0, &zErrMsg);
	...
	sprintf(sql_buff,"insert into TAG_USER_DATA (tag_val, tag_antID, tag_action, tag_first_seen, tag_last_seen, tag_seen_count)" \
			 "VALUES" \
	         "('%s', %d, %d, datetime('now'), datetime('now'), 1);",tag, antid, action); 
...
int
CSqlite::user_tag_action(char *tag, int antid, int *action)
{
	int rc;
	
	sprintf(sql_buff, "select tag_id,tag_action,tag_seen_count from TAG_USER_DATA where tag_val='%s' AND tag_antID=%d;", tag, antid);
